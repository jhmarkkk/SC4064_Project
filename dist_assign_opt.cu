...
// Transpose AoS to SoA for a specific chunk
__global__ void convertAosToSoA_Chunk_Kernel(const float* __restrict__ input_AoS, 
                                             float* __restrict__ output_SoA, 
                                             int chunk_size, int D) {
    // TILE_DIM + 1 avoids shared memory bank conflicts
    __shared__ float tile[TILE_DIM][TILE_DIM + 1]; 

    int x = blockIdx.x * TILE_DIM + threadIdx.x; // Maps to Dimension (D)
    int y = blockIdx.y * TILE_DIM + threadIdx.y; // Maps to Point Index within chunk

    // 1. Read from Global Memory (AoS) into Shared Memory
    if (x < D && y < chunk_size) {
        tile[threadIdx.y][threadIdx.x] = input_AoS[y * D + x];
    }

    __syncthreads();

    // 2. Write from Shared Memory to Global Memory (SoA format for this chunk)
    x = blockIdx.y * TILE_DIM + threadIdx.x; // Now X maps to Point Index
    y = blockIdx.x * TILE_DIM + threadIdx.y; // Now Y maps to Dimension

    if (x < chunk_size && y < D) {
        output_SoA[y * chunk_size + x] = tile[threadIdx.x][threadIdx.y]; 
    }
}
...
int main() {
    ...
    std::cout << "--- Launch Configuration (Optimized, custom transpose) ---" << std::endl;
    ...
    for (int i = 0; i < NUM_STREAMS; ++i) {
        ...
        // Grid parameters
        dim3 blockTranspose(TILE_DIM, TILE_DIM);
        dim3 gridTranspose((D + TILE_DIM - 1) / TILE_DIM, (current_chunk_size + TILE_DIM - 1) / TILE_DIM);
        ...
        // Step B: Transpose this chunk from AoS to SoA using custom kernel
        convertAosToSoA_Chunk_Kernel<<<gridTranspose, blockTranspose, 0, streams[i]>>>(
            d_points_AoS + float_offset,
            d_points_SoA + float_offset, 
            current_chunk_size, D);
    ...
    std::cout << "\n--- Performance Metrics (Optimized, custom transpose) ---" << std::endl;
